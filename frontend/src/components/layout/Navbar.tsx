...
export default function Navbar() {
  ...
  return (
    <nav className={`sticky top-0 z-50 bg-white transition-shadow duration-200 ${scrolled ? 'shadow-md border-b border-rose-100' : 'border-b border-rose-100'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Carrito */}
            <Link to="/cart" className="relative p-2 text-stone-400 hover:text-rose-400 transition-colors">
              <ShoppingCart size={21} />
              {totalUnits > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-rose-400 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center font-bold leading-none">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-1">
                {isAdmin && (
                  <Link
                    to="/admin"
                    className="hidden sm:flex items-center gap-1.5 text-xs font-semibold bg-rose-50 text-rose-500 hover:bg-rose-100 px-3 py-1.5 rounded-lg transition-colors border border-rose-200"
                  >
                    <LayoutDashboard size={12} /> Admin
                  </Link>
                )}
                <Link to="/profile" className="p-2 text-stone-400 hover:text-rose-400 transition-colors">
                  <User size={21} />
                </Link>
                <button onClick={handleLogout} className="p-2 text-stone-300 hover:text-red-400 transition-colors">
                  <LogOut size={19} />
                </button>
              </div>
            ) : (
      ...
      {/* ── Menú mobile ── */}
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-rose-50 px-6 py-5 flex flex-col gap-4 text-sm font-medium text-stone-500">
